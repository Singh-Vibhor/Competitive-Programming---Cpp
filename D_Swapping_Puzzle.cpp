...
void col_recur(vector<vector<ll> >&a, vector<vector<ll>> &b, ll rcnt, ll ccnt, ll rowop, ll colop){
    // debug(ans)
    if(a==b){
        ans=min(ans,rcnt+ccnt);
        return;
    }
    ...
    // swap columns i and j of a
    for(int i=0;i<a[0].size();i++){
        for(int j=i+1;j<a[0].size();j++){
            for(int k=0;k<a.size();k++){
                swap(a[k][i],a[k][j]);
            }
            col_recur(a,b,rcnt,ccnt+j-i+j-i-1,rowop,colop+1);
            for(int k=0;k<a.size();k++){
                swap(a[k][i],a[k][j]);
            }
        }
...
void rowrecur(vector<vector<ll> >&a, vector<vector<ll>> &b, ll rcnt, ll ccnt, ll rowop, ll colop){
    // debug(ans)
    if(a==b){
        ans=min(ans,rcnt+ccnt);
        return;
    ...
    col_recur(a,b,rcnt,ccnt,rowop,colop);
    ...
    for(int i=0;i<a.size();i++){
        for(int j=i+1;j<a.size();j++){
            ...
            rowrecur(a,b,rcnt+j-i+(j-i-1),ccnt,rowop+1,colop);
            for(int k=0;k<a[0].size();k++){
                swap(a[i][k],a[j][k]);
            }
        }
